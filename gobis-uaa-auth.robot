...
...               cf  delete  %{CF_BINARY_APP_NAME}   -f
...               AND  cf  unbind-route-service  %{CF_DOMAIN}  --hostname  %{CF_BINARY_APP_NAME}  ${uaa_auth_instance_name}   -f
...               AND  cf  delete-service  ${uaa_auth_instance_name}  -f
Default Tags      gobis   uaa-auth
*** Test Cases ***
User auth can be set on an app and unset
    Given I push default binary app named %{CF_BINARY_APP_NAME} and expect response
    When I set up uaa-auth user route service on app %{CF_BINARY_APP_NAME}
    Then I expect to be redirected to login page from app %{CF_BINARY_APP_NAME}

    When I unset uaa-auth user route service on app %{CF_BINARY_APP_NAME}
    Then I expect app %{CF_BINARY_APP_NAME} from instance 0 to contains response "Hello from a binary"
*** Variables ***
${uaa_auth_instance_name}     my-uaa-auth
